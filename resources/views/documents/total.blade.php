...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="stats-card stats-card-dark">
            ...
            <div class="number">{{ $allDocuments->count() }}</div>
            <div class="subtitle">All documents in the system</div>
        </div>
    </div>
</div>
                ...
                <table class="table table-striped" id="documents-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $doc->version }}</td>
                            <td>
                                <a href="{{ route('uploads.revisions', $doc->upload_id) }}?return_to={{ url()->full() }}" class="btn btn-sm btn-secondary">
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton{{ $doc->id }}">
                                        <a class="dropdown-item" href="{{ $doc->upload_id ? route('uploads.view', ['upload_id' => $doc->upload_id]) : '#' }}">
                                            <i class="fas fa-eye mr-2"></i> View
                                        </a>
                                        @if(!empty($doc->filename))
                                            <a class="dropdown-item" href="{{ route('documents.download', ['file' => urlencode($doc->filename), 'id' => $doc->upload_id]) }}">
                                                <i class="fas fa-download mr-2"></i> Download
                                            </a>
                                        @endif
                                        @if(Auth::user()->role == 0 || Auth::user()->role == 1 || Auth::user()->role == 2)
                                            <a class="dropdown-item" href="{{ route('uploads.edit', $doc->upload_id) }}">
                                                <i class="fas fa-edit mr-2"></i> Edit
                                            </a>
                                            <!-- ✅ Delete opens modal -->
                                            <a class="dropdown-item text-danger" href="#" data-document-id="{{ $doc->upload_id }}">
