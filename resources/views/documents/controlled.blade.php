...
<div class="dashboard-header">
    <div>
        ...
        <h1 class="mt-3">Controlled Documents</h1>
        <p>Overview of controlled documents</p>
    </div>
    <div class="search-box">
        <div class="input-group" style="width: 400px;">
            ...
            <input type="text" class="form-control" placeholder="Search controlled documents...">
        </div>
...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="stats-card controlled">
            ...
            <div class="title">Controlled Documents</div>
            <div class="number">{{ count($controlledDocumentsList) }}</div>
            <div class="subtitle">Documents marked as controlled</div>
        </div>
    </div>
</div>
...
<div class="row mt-4">
    <div class="col-md-12">
        ...
        <div class="documents-table">
            ...
            <p>A comprehensive list of controlled documents</p>
            <div class="table-responsive">
                <table class="table table-striped" id="documents-table">
                    ...
                    <tbody>
                        @foreach($controlledDocumentsList as $doc)
                            <tr>
                                <td>{{ $doc->title }}</td>
                                <td>{{ $doc->control_number }}</td>
                                <td>{{ $doc->type }}</td>
                                <td>
                                    <span class="status-badge controlled">
                                        Controlled
                                    </span>
                                </td>
                                <td>{{ $doc->version }}</td>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton{{ $doc->id }}">
                                                <a class="dropdown-item" href="{{ $doc->upload_id ? route('uploads.view', ['upload_id' => $doc->upload_id]) : '#' }}"><i class="fas fa-eye mr-2"></i> View</a>
                                                @if(!empty($doc->filename))
                                                    <a class="dropdown-item" href="{{ route('documents.download', ['file' => urlencode($doc->filename), 'id' => $doc->upload_id]) }}"><i class="fas fa-download mr-2"></i> Download</a>
                                                @endif
                                                ...
                                                <a class="dropdown-item text-danger" href="#" data-toggle="modal" data-target="#deleteConfirmationModal" data-document-id="{{ $doc->upload_id }}"><i class="fas fa-trash-alt mr-2"></i> Delete</a>
                                                @endif
...
<div class="modal fade" id="deleteConfirmationModal" tabindex="-1" role="dialog" aria-labelledby="deleteConfirmationModalLabel" aria-hidden="true">
    ...
    $('#deleteConfirmationModal').on('show.bs.modal', function (event) {
        ...
        var documentId = button.data('document-id'); // Extract info from data-* attributes
        var form = $('#deleteDocumentForm');
        form.attr('action', '/uploads/' + documentId);
    });
                </button>
            </div>
